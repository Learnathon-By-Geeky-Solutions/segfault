...
const PROTO_PATHS = [
    ...
    path.join(__dirname, "../src/proto/submission_process.proto"),
];
...
const
    [hiddenTestProcessProto,
    referenceSolutionValidationProcessProto,
    submissionProcessProto
    ] = loadProtos(PROTO_PATHS);

const hiddenTestProcessService = hiddenTestProcessProto.hidden_test_process.HiddenTestProcess.service;
const referenceSolutionValidationProcessService = referenceSolutionValidationProcessProto.ref_sol_validation_process.RefSolValidationProcess.service;
const submissionProcessService = submissionProcessProto.submission_process.SubmissionProcess.service;
...
const streamReferenceSolutionValidationProcess = async (call: any, callback: Function) => {
    console.log(`Metadata: ${JSON.stringify(call.metadata)}`);
    const clientId = call.metadata.get("client_id")[0];
...
const streamSubmissionProcess = async (call: any, callback: Function) => {
    console.log(`Metadata: ${JSON.stringify(call.metadata)}`);
    const clientId = call.metadata.get("client_id")[0];
...
server.addService(referenceSolutionValidationProcessService,
    {streamRefSolValidationProcess: streamReferenceSolutionValidationProcess});
server.addService(submissionProcessService, {streamSubmissionProcess});
// Start the server
